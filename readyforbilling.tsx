...
// --- COMPONENTS ---
const ExpandedBillingView = ({ order, billedQty, onQtyChange, onMarkBilled, isProcessing }) => {
...
export const ReadyForBilling = () => {
    ...
    const handleMarkBilled = async (order, billedQuantities) => {
        ...
        try {
            ...
            updates[billedOrderRefPath] = { ...order, items: finalBilledItems, totalQuantity: finalBilledItems.reduce((sum, i) => sum + i.quantity, 0), totalValue: finalBilledItems.reduce((sum, i) => sum + (i.quantity * i.price), 0), status: 'Billed', billedTimestamp: new Date().toISOString() };
            if (itemsRemainingInBilling.length > 0) {
                updates[billingOrderRefPath] = { ...order, items: itemsRemainingInBilling, totalQuantity: itemsRemainingInBilling.reduce((sum, i) => sum + i.quantity, 0), totalValue: itemsRemainingInBilling.reduce((sum, i) => sum + (i.quantity * i.price), 0), };
            } else {
